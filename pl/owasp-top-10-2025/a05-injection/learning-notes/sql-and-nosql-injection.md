...
- Raw input może być właściwy w UI, które samo wykonuje encoding.
- Encoded characters są potrzebne przy bezpośredniej edycji requestu.
- W `application/x-www-form-urlencoded` znak `+` oznacza spację.
- `%3B` oznacza `;`.
- Niezakodowany `&` może rozdzielić parametry formularza albo query, zanim zamierzona wartość trafi do backendu.
