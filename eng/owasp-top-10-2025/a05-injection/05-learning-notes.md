...
SQL Injection, NoSQL Injection, and OS Command Injection all reinforce the wider A05 model:

```text
controlled input -> interpreter or execution mechanism -> changed behaviour
```

The payload is not the root cause by itself. The vulnerability exists because the application allows untrusted input to become query syntax, a query operator, part of an executable expression, shell syntax, or a dangerous process argument.
...
- what parser, query engine, shell, or process API handles it,
